...
<p>The first step is to delete the <strong>'attempts += 1'</strong> under <strong>password = input("Enter your password: ")</strong> as it is not needed because it'll start counting before checking if the password is correct or not. Moreover, it counts every input as a failed attempt, even the correctly guessed password.</p>

<p>Second step is to change the <strong>if password == "incorrect_password":</strong> because that isn't the correct password given to us in the code. Instead we change it to <strong>if password == correct_password:</strong>.</p>
